...
\iftrue % use default-font
  \ifluatex
    \lmoderntrue
    % select arbitrary font. Will be replaced by latin modern right after "\begin{document}"
...
\else
  ...
  \ifluatex
    ...
    \usepackage[varl,scaled=.9]{inconsolata}
    \lmodernfalse
  \else
    \usepackage{newtxtext}
    \usepackage{newtxmath}
...
\begin{document}
% This line ensures that lmodern is used. Do not remove it.
% hint by https://tex.stackexchange.com/a/25006/9075 -> switch back to latin modern
\ifluatex\iflmodern\renewcommand\rmdefault{lmr}\renewcommand\sfdefault{lmss}\renewcommand\ttdefault{lmtt}\fi\fi
...
``something in quotes'' using plain tex currently does not work, because of \href{https://github.com/micoloretan/selnolig/issues/8}{selnolig\#8}.
...
In case Lua\LaTeX{} is used as compiler, there is no ligature at \enquote{f"|l} in the word \enquote{dwarflike} (in contrast to \enquote{fl} at \enquote{workflow}).
In other words: \enquote{dwarflike} and \enquote{dwarf"|like} look the same in the PDF.
